...
use crate::{
  state::{Vault, VaultStatus, User, ErrorCode},
  constant::{VAULT_REWARD_SEED},
...
pub fn claim(ctx: Context<Claim>) -> Result<()> {
  ...
  if user.reward_earned_pending > 0 {
    ...
    if reward_amount > 0 {
      ...
      anchor_spl::token::transfer(cpi_context.with_signer(&[&seeds[..]]), reward_amount)?;

      // update reward claiming values
