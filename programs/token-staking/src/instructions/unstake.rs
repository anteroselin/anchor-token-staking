...
use crate::state::{Vault, VaultStatus, ErrorCode, User};
...
pub struct Unstake<'info> {
  ...
  #[account(
    mut,
    seeds = [VAULT_STAKE_SEED.as_bytes(), vault.key().as_ref(), staker.key().as_ref()],
...
pub fn unstake(ctx: Context<Unstake>, _vault_stake_bump: u8) -> Result<()> {
  ...
  if !user.mint_accounts.iter().any(|x| *x == unstake_account.key()) {
    ...
    let index = user.mint_accounts.iter().position(|x| *x == unstake_account.key());
